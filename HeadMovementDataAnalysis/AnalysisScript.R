...
for(i in 1:9)
{
  ...
  for(i in 1:ParticipantNumber)
  {
    for(j in 1:DataSetsPerPartcipant)
    {
      ...
      plot.ts(ImportedData[ColumnToPlot], ylab="plot.ts(ImportedData[ColumnToPlot])", xlab="Data Ticks") #Plots the time sereis for the selected data.
  ...
  for(i in 1:ParticipantNumber)
  {
    for(j in 1:DataSetsPerPartcipant)
    {
      ...
      png(file = mypath)
      plot.ts(diff(ImportedData[,c(ColumnToPlot)]), xlab="Data Ticks")#Plots the difference between data recordings.
      dev.off()
  ...
  for(i in 1:ParticipantNumber)
  {
    for(j in 1:DataSetsPerPartcipant)
    {
      ...
      png(file = mypath)
      plot.ts(diff(diff(ImportedData[,c(ColumnToPlot)])), xlab="Data Ticks")#Plots the difference of the difference between data recordings.
      dev.off()
      ColumnToPlot <- ColumnToPlot + 1
    }
  }
